 ...
 *
 *   node --import tsx/esm ./tools/lint-corpus.mts [--spec <file>] [--verbs get|mutations|all] [--v04]
 *
 * This branch generates connect v0.5 with reusable `@mapping` by default, which is what it exists
 * for. `--v04` falls back to the shipping version, for comparing against main.
 */
import fs from 'fs';
...
function reportName(): string {
  ...
  return `LINT-CORPUS${specPart}${verbsPart}${v05 ? '' : '-v04'}.md`;
}
...
function report(): string {
  const lines = [
    ...
    '',
    `Generated by \`tools/lint-corpus.mts${v05 ? '' : ' --v04'}${verbs === 'get' ? '' : ` --verbs ${verbs}`}\`.`,

// Corpus coverage harness. For each vendor spec, iterate EVERY GET op and, under two generator
// configs — both at the current shipping versions (connect v0.4 / fed 2.14), differing only in the
// union strategy — try to generate + rover-compose it. Classifies each outcome and writes COVERAGE.md
// (per-spec pass-rates + a global failure histogram = prioritized gap list).
//
...
Generated by \`tools/coverage-spec.mts\`. For each vendor spec, **every GET op** is generated and
rover-composed once (real unions, the shipping default). This is the real coverage measure behind the single-path
\`test_corpus_*\` smoke tests. **† = loaded with \`skipValidation\`** (patched fixture; see TEST_CORPUS.md).
...
(incl. GEN-HANG, a sync infinite loop) · **COMPOSE-fail** rover rejected the schema. pass-rate = OK / GET ops.

...
// #160: --note-response-fields adds the top-level field names of an operation's response to its
...
const run = (paths: string[], opts: { noteResponseFields?: boolean; keepFieldNames?: boolean } = {}) =>
...
test('test_160_object_response', async () => {
  ...
  const schema = await run(GET_ITEM_PATHS, { noteResponseFields: true });
  assert.ok(
...
test('test_160_array_response', async () => {
  ...
  const schema = await run(LIST_ITEMS_PATHS, { noteResponseFields: true });
  assert.ok(
...
test('test_160_caps_at_14_fields', async () => {
  ...
  const schema = await run(WIDE_PATHS, { noteResponseFields: true });
  assert.ok(
    schema!.includes(
...
test('test_160_uses_kept_spelling_with_158', async () => {
  ...
  const schema = await run(GET_ITEM_PATHS, { noteResponseFields: true, keepFieldNames: true });
...
test('test_160_mutation_verbs', async () => {
  ...
  const schema = await run(MUTATION_PATHS, { noteResponseFields: true });
...
test('test_160_cli_flag_reaches_generator', () => {
  ...
  const cli = spawnSync(
    'node',
    ['--import', 'tsx/esm', 'src/cli/oas.ts', 'tests/resources/oas/doc-response-fields.yaml', '-i', '-n', '--note-response-fields'],
    { encoding: 'utf-8' },
  );
...
test('test_160_old_switch_name_still_works', () => {
  // Checks the old --doc-response-fields still writes the Returns line, through the CLI, and names the new switch
  const cli = spawnSync(
    'node',
  ...
  assert.strictEqual(cli.status, 0, cli.stderr);
  assert.ok(cli.stdout.includes('Returns a list of items with: createdAt, id, name'), 'sets noteResponseFields');
  assert.ok(cli.stderr.includes('--doc-response-fields is now --note-response-fields'), 'warns with the new name');
});

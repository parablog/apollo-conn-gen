...
test('test_115_enum_dedup_holds_on_ref_component_and_input_routes', async () => {
  // #115: #102's dedup was only pinned for an inline enum property. A $ref'd component enum and
  // its input-position reuse pass through the same En constructor — pin them so they stay that way.
  const schema = await runOasTest('duplicate-enum-values-routes.yaml', ['get:/jobs>**', 'post:/jobs>**'], 3, 3);
  assert.ok(schema !== undefined);
  assert.strictEqual((schema!.match(/enum StateCode \{/g) || []).length, 1, 'one shared enum definition');
  const block = schema!.match(/enum StateCode \{[^}]*\}/)?.[0] ?? '';
  assert.strictEqual((block.match(/\bactive\b/g) || []).length, 1, 'repeated value written once');
  assert.strictEqual((block.match(/\bdone\b/g) || []).length, 1, 'both repeated values written once');
  assert.ok(/pending[\s\S]*active[\s\S]*done/.test(block), 'listed order is kept');
  assert.ok(/state: StateCode/.test(schema!), 'output side references the enum');

...
test('test_R10_leaf_type_gets_automap_mapping', async () => {
  ...
  // (bare @mapping, no selection argument), and the @connect selection just applies it.
  ...
  assert.ok(schema!.includes('$->User'), 'the @connect selection should apply the type mapping');
});
...
test('test_R10_nested_type_gets_explicit_mapping_with_spreads', async () => {
  ...
  assert.ok(schema!.includes('category: category->Category'), 'nested object field applies the child mapping');
  assert.ok(schema!.includes('tags: tags->Tag'), 'array-of-object field applies the item mapping');
  assert.ok(schema!.includes('$->Pet'), 'the @connect selection should apply the type mapping');
});
...
test('test_R10_entity_resolver_spreads_with_mapping', async () => {
  ...
  assert.ok(/\$->User/.test(schema!), 'entity selection should apply the type mapping');
});
...
test('test_R10_two_type_cycle_stays_acyclic', async () => {
  ...
  // exactly one direction applies the mapping, the cycle-closing reference renders as the existing
  ...
  assert.ok(aSpreadsB !== bSpreadsA, 'exactly one direction may apply the mapping — both would loop the router expander');
});

test('test_R10_three_type_cycle_stays_acyclic', async () => {
  // X -> Y -> Z -> X: same invariant on a longer cycle — the mapping chain must be broken once.
  ...
  assert.ok(spreadEdges.filter(Boolean).length >= 1, 'non-closing edges still apply the mapping');
});

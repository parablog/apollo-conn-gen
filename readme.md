...
Selections mark OAS-optional fields with the mapping language's `?` (`name?`, `category? { … }`), so a key the API legitimately omits stops triggering the router's missing-property warnings; required fields stay plain, and entity keys are never marked. Composing schemas with these markers needs **composition 2.15 or newer** (`federation_version: =2.15.1` in a supergraph config) — the latest stable composition release, `2.14.3`, fails them with `CONNECTORS_UNRESOLVED_FIELD`. Pass `--skip-optional-markers` (library: `skipOptionalMarkers: true`) to leave the markers out and compose with `federation_version: =2.14.3`; markers are emitted by default. `--skip-optional-markers` only removes the `?` marker — it does nothing for the map or default-fallback syntax below.

**Recommended floor: composition plugin / router 2.15 or newer.** A spec with any map (dictionary) value fails composition below 2.15 with no escape flag — real specs measured 322-361 `CONNECTORS_UNRESOLVED_FIELD` errors at plugin `2.14.0` (`docs/FIXED.md #108`, `#109`). Default-value fallbacks (`tag: tag ?? $("latest")`, ROADMAP R7) compose in isolation on `2.14.1`, `2.14.3`, and `2.15.1`, but not on `2.14.0`, which rejects them with `INVALID_SELECTION` (a `nom` parser error) — measured directly (`docs/FIXED.md #163`, `tests/all/oas-core.test.ts` `test_163_*`). A router deployed at exactly `2.14.0` also crashed at startup on this same syntax in the field (`nom::ErrorKind::Eof`), so 2.15+ is the conservative recommendation to plan around rather than a per-patch minimum measured against a deployed router.
